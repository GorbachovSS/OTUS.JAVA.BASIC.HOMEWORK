...
public class EverywhereRunning implements Transport {
    private Human human;
    private int petrol;

    public EverywhereRunning(int petrol) {
        this.petrol = petrol;
    }
    ...
    public boolean moving(int distance, Terrain terrain) {
        if (terrain == Terrain.PLAIN && petrol >= distance) {
            return true;
        } else if (terrain == Terrain.SWAMP && petrol / 3 >= distance) {
            return true;
        } else if (terrain == Terrain.DENSE_FOREST && petrol / 2 >= distance) {
            return true;
        } else {
            return false;
        }
